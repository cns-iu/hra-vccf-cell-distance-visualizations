...
folder_path = r'G:\HuBMAP\Hickey\Intestine_64_data\vitessce_raw'

union_of_types = set()

# Iterate through the files and read the unique values in the "type" column
for i in range(1, 65):
    file_path = os.path.join(folder_path, f'Region_{i}_nuclei.csv')
...
# Add keys with '_link' suffix, and assign the same value as the original key
for key, value in list(color_dict.items()):
    color_dict[key + '_link'] = value
