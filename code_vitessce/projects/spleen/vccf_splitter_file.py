...
root_path = r"G:\HuBMAP\spleen"
file_name = r"FSLD.meta.csv"
file_label = file_name.split('.')[0]
split_source_file = os.path.join(root_path, file_name)

data_new = pd.read_csv(split_source_file, sep=';')
...
cell_types = data_new["celltypes"].unique()
print(cell_types)
...
split_target_file = f"{split_source_file.replace(file_name, 'new_data/Region')}_{file_label}.csv"

# Write to the file using pandas to_csv
data_new.to_csv(split_target_file, index=False, header=True, mode="w")
...
root_path = r"G:\HuBMAP\spleen\new_data"
path = root_path + rf"/Region_{file_label}.csv"
df_Region_1 = pd.read_csv(path)

# Get coordinates x, y, Cell subtype and cellType (2D Data)
df_Region_1 = df_Region_1[["x", "y", "celltypes"]]

# rename column cell_type_A to Cell Type
df_Region_1.rename(columns={"celltypes": "Cell Type"}, inplace=True)
...
df_Region_1_vessel = df_Region_1.loc[df_Region_1["Cell Type"] == 'Blood endothelial']
df_Region_1_immmune = df_Region_1.loc[df_Region_1["Cell Type"] != 'Blood endothelial']
...
df.to_csv(
    os.path.join(root_path, "vitessce_raw", f"Region_{file_label}_vessels.csv"),
    index=False,
)
...
df.to_csv(
    os.path.join(root_path, "vitessce_raw", f"Region_{file_label}_nuclei.csv"),
    index=False,
)